...
ursa.token.rule{"opengal_files", nil, function () return ursa.system{("cd %s && find . -type f | sed s*\\\\./** | grep -v \\.git"):format(path)} end}
for item in ursa.token{"opengal_files"}:gmatch("([^\n]+)") do
  table.insert(files, ursa.rule{builddir .. "lib_build/opengal32/" .. item, path .. "/" .. item, ursa.util.copy{}})
...
if platform == "cygwin" then
  ...
  ursa.rule{builddir .. "lib_release/bin/OpenGAL32-1.dll", builddir .. "lib_build/opengal32/build/OpenGAL32-1.dll", ursa.util.copy{}}
else
  local ext = "so"
  local po = "OFF"
  ...
  local lib = ursa.rule{builddir .. "lib_build/opengal32/build/libopengal." .. ext, {files, headers.portaudio, libs.portaudio}, ursa.util.system_template{('cd %slib_build/opengal32/build && CFLAGS="#CCFLAGS -I%s/build/%s/lib_release/include" CXXFLAGS="#CXXFLAGS -I%s/build/%s/lib_release/include" LDFLAGS="#LDFLAGS -L/usr/lib -L%s/build/%s/lib_release/lib" cmake -DCMAKE_BUILD_TYPE=Release DSOLARIS=OFF -DOSS=OFF -DWINMM=OFF -DPORTAUDIO=%s -DPULSEAUDIO=OFF -DEXAMPLES=OFF .. && make'):format(builddir, ursa.token{"PWD"}, platform, ursa.token{"PWD"}, platform, ursa.token{"PWD"}, platform, po)}}
  
  if platform == "osx" then
    libs.opengal = ursa.rule{builddir .. "lib_release/lib/libopengal.dylib", builddir .. "lib_build/opengal32/build/libopengal.dylib", ursa.util.copy{}}
    ursa.rule{builddir .. "lib_release/bin/libopengal.dylib", builddir .. "lib_build/opengal32/build/libopengal.dylib", ursa.util.copy{}}
  else
    libs.opengal = ursa.rule{builddir .. "lib_release/lib/libopengal.so", builddir .. "lib_build/opengal32/build/libopengal.so", ursa.util.copy{}}
    ursa.rule{builddir .. "lib_release/bin/libopengal.so", builddir .. "lib_build/opengal32/build/libopengal.so", ursa.util.copy{}}
  end
end
