...
for file in ursa.token{"libpng files"}:gmatch("([^\n]+)") do
  table.insert(headers.libpng, ursa.rule{builddir .. "lib_build/libpng/" .. file, libpng_dir .. "/" .. file, ursa.util.copy{}})
end

local makefile = ursa.rule{{builddir .. "lib_build/libpng/Makefile"}, {headers.libpng, headers.zlib, libs.zlib}, ursa.util.system_template{('cd %s/lib_build/libpng && CC=#CC CFLAGS="#CCFLAGS" LDFLAGS="#LDFLAGS" ./configure --disable-dependency-tracking'):format(builddir)}}

local lib = ursa.rule{builddir .. "/lib_build/libpng/.libs/libpng14.a", {makefile, headers.libpng}, ('cd %s/lib_build/libpng && make -j5'):format(builddir)}
