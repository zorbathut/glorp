...
//#define OPENAL_DEMO
  ...
  */
  ALboolean LoadALData()
  {
    ...
    ALsizei size = 44100 * 4;
    short *data = new short[44100 * 2];
    ALsizei freq = 44100;
    ALboolean loop = AL_TRUE;

    for(int i = 0; i < 44100; i++) {
      short val = sin(i * 3.14159 / 100 * 2) * 30000;
      data[i * 2] = val;
      data[i * 2 + 1] = val;
    }
  ...
  */
  ...
  void print_devices(const char* device_list) 
{ 
        bool separator = false; 
        char device[MAX_LENGTH]; 
        int list_pos = 0, pos = 0; 


        if (device_list == NULL) 
        { 
                dprintf("WARNING: Device list is null (%s:%d).\n", __FILE__, __LINE__); 
                return; 
        } 
        ...
        { 
                if (pos > MAX_LENGTH) 
                { 
                        dprintf("WARNING: Device name too large (%s:%d).\n", __FILE__, __LINE__); 
                        return; 
                } 

                separator = device_list[list_pos] == 0; 
                if (separator) 
                { 
                        device[pos] = 0; 
                        dprintf("Device: %s\n", device); 
                ...
                if (separator && device_list[list_pos + 1] == 0) 
                        break; 

                device[pos++] = device_list[list_pos++]; 
        } 
        while (true); 
} 

  void openal_demo_core() {
    const char* device_list;
            if (alcIsExtensionPresent(NULL, "ALC_ENUMERATION_EXT")) 
        { 
                dprintf("Supports: ALC_ENUMERATION_EXT\n"); 
                device_list = (char*)alcGetString(NULL, ALC_DEVICE_SPECIFIER); 
        } 
        /* if (alcIsExtensionPresent(NULL, "ALC_ENUMERATE_ALL_EXT")) 
        { 
                dprintf("Supports: ALC_ENUMERATE_ALL_EXT\n"); 
