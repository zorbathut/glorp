...
for file in ursa.token{"libjpeg files"}:gmatch("([^\n]+)") do
  table.insert(headers.libjpeg, ursa.rule{builddir .. "lib_build/libjpeg/" .. file, libjpeg_dir .. "/" .. file, ursa.util.copy{}})
end

local makefile = ursa.rule{{builddir .. "lib_build/libjpeg/Makefile", builddir .. "lib_build/libjpeg/jconfig.h"}, headers.libjpeg, ursa.util.system_template{('cd %s/lib_build/libjpeg && CC=#CC CFLAGS="#CCFLAGS" LDFLAGS="#LDFLAGS" ./configure --disable-dependency-tracking'):format(builddir)}}

local lib = ursa.rule{builddir .. "lib_build/libjpeg/.libs/libjpeg.a", {makefile, headers.libjpeg}, ('cd %s/lib_build/libjpeg && make clean && make -j5'):format(builddir)}
