...
for file in ursa.token{"libfreetype files"}:gmatch("([^\n]+)") do
  if file == "builds/unix/unix-def.in" then
    table.insert(headers.libfreetype, ursa.rule{builddir .. "lib_build/libfreetype/" .. file, libfreetype_dir .. "/" .. file, ursa.util.system_template{'sed "s/TOP_DIR := .*/TOP_DIR := ./" $SOURCE > $TARGET'}})
  else
    table.insert(headers.libfreetype, ursa.rule{builddir .. "lib_build/libfreetype/" .. file, libfreetype_dir .. "/" .. file, ursa.util.copy{}})
  end
...
local makefile = ursa.rule{{builddir .. "lib_build/libfreetype/builds/unix/freetype-config"}, headers.libfreetype, ursa.util.system_template{('cd %s/lib_build/libfreetype && CC=#CC CFLAGS="#CCFLAGS" LDFLAGS="#LDFLAGS" ./configure'):format(builddir)}}

local lib = ursa.rule{builddir .. "lib_build/libfreetype/objs/.libs/libfreetype.a", {makefile, headers.libfreetype}, ('cd %s/lib_build/libfreetype && make -j5'):format(builddir)}
